...
bool MC2EdenConverter::convertRegionFolderToEden(const std::string& inputRegionFolder,
                                                   const std::string& outputEdenPath) {
    AnvilReader reader;
    ...
    if (chunks.empty()) {
        std::printf("No chunks read from: %s\n", inputRegionFolder.c_str());
        return false;
    }
    ...
    std::printf("Downsampled to %zu Eden columns.\n", edenColumns.size());
