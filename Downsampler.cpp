...
namespace {
static std::string modeBlockName4(const std::string& a, const std::string& b,
    ...
    for (const auto& kv : counts) {
        ...
        if (kv.second > nonAirBest) { nonAirBest = kv.second; nonAirName = kv.first; }
    }
    return (nonAirBest > 0) ? nonAirName : "minecraft:air";
}
} // namespace
