...
struct ByteReader {
    ...
    uint32_t be32() { uint32_t v = (uint32_t)(u8() << 24); v |= (uint32_t)(u8() << 16);
                      v |= (uint32_t)(u8() << 8); v |= u8(); return v; }
    uint64_t be64() { uint64_t v = 0; for (int i = 0; i < 8; ++i) v = (v << 8) | (uint64_t)u8(); return v; }
    std::string str() {
        uint16_t n = be16();
        if (!has(n)) return "";
        std::string s(reinterpret_cast<const char*>(&b[p]),
                      reinterpret_cast<const char*>(&b[p + n]));
        p += n; return s;
    }
...
static std::vector<std::string> readPaletteList(ByteReader& br) {
    ...
    for (uint32_t i = 0; i < len; ++i) {
        ...
        while (br.has(1)) {
            ...
            if (t == TAG_String && tagName == "Name") name = br.str();
            else skipNamedTagPayload(br, t);
        }
...
static void parseBlockStatesCompound(ByteReader& br, SectionData& section) {
    while (br.has(1)) {
        ...
        if (t == TAG_End) break;
        std::string n = br.str();
        if (t == TAG_List && n == "palette")           section.palette   = readPaletteList(br);
        else if (t == TAG_Long_Array && n == "data")   section.stateData = readLongArray(br);
        else                                           skipNamedTagPayload(br, t);
    }
}
...
static void parseChunkCompound(ByteReader& br, ParsedChunk& outChunk) {
    while (br.has(1)) {
        ...
        if (t == TAG_End) break;
        std::string n = br.str();
        if (t == TAG_Int && (n == "xPos" || n == "xpos"))       outChunk.xPos = (int32_t)br.be32();
        else if (t == TAG_Int && (n == "zPos" || n == "zpos"))  outChunk.zPos = (int32_t)br.be32();
        else if (t == TAG_List && (n == "Sections" || n == "sections")) parseSectionsList(br, outChunk);
        else if (t == TAG_Compound && n == "Level")             parseChunkCompound(br, outChunk);
        else                                                    skipNamedTagPayload(br, t);
    }
}
...
static std::string unpackPaletteName(const SectionData& s, int blockIndex) {
    ...
    int bits = 4;
    while ((1U << bits) < (int)s.palette.size()) bits++;
    ...
    return (idx < s.palette.size()) ? s.palette[idx] : "minecraft:air";
}
...
static Chunk toChunk(const ParsedChunk& pc) {
    ...
    for (int x = 0; x < 16; ++x)
        for (int y = 0; y < 256; ++y)
            for (int z = 0; z < 16; ++z)
                out.blocks[x][y][z].blockName = "minecraft:air";
...
static std::vector<ChunkColumn> readSingleRegion(const std::string& mcaPath) {
    ...
    for (int i = 0; i < 1024; ++i) {
        uint32_t loc = ((uint32_t)bytes[i * 4 + 0] << 24)
        ...
        if (offset + span > bytes.size() || span < 5) continue;
        uint32_t length = ((uint32_t)bytes[offset + 0] << 24)
        ...
        std::vector<uint8_t> compressed(bytes.begin() + (long)(offset + 5),
                                        bytes.begin() + (long)(offset + 4 + length));
        std::vector<uint8_t> nbt;
        if (compression == 1 || compression == 2) {
            if (!inflateAuto(compressed, nbt)) {
                std::printf("Decompress failed in %s chunk %d\n", mcaPath.c_str(), i);
                continue;
            }
        } else {
            std::printf("Unsupported compression %u in %s\n", compression, mcaPath.c_str());
            continue;
        }

        ParsedChunk parsed;
        if (!parseChunkNbt(nbt, parsed)) continue;
...
std::vector<ChunkColumn> AnvilReader::readRegionFolder(const std::string& inputRegionFolder) {
    ...
    if (!fs::exists(inputRegionFolder) || !fs::is_directory(inputRegionFolder)) {
        std::printf("Region folder not found: %s\n", inputRegionFolder.c_str());
        return all;
    }
...
bool AnvilReader::debugPrintFirstChunk(const std::string& mcaPath) {
    ...
    if (chunks.empty()) { std::printf("No readable chunks in %s\n", mcaPath.c_str()); return false; }
    const ChunkColumn& cc = chunks.front();
    std::printf("Debug chunk (%d,%d):\n", cc.chunkX, cc.chunkZ);
    for (int y = 63; y >= 56; --y) {
        std::printf("y=%d ", y);
        for (int x = 0; x < 8; ++x) {
